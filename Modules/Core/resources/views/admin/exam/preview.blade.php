...
@section('title', 'Aperçu de l\'Examen - ' . $exam->title)
...
<div class="preview-container">
  ...
  <div class="simulator-toolbar d-flex align-items-center justify-content-between p-3 bg-white border-bottom shadow-sm">
    <div class="d-flex align-items-center gap-2">
      <a href="{{ route('cores.editor.exams.edit', $exam->id) }}" class="btn btn-sm btn-outline-secondary" style="border-radius: 8px;">
      ...
      <h5 class="mb-0 ms-2 fw-bold text-truncate" style="max-width: 250px; color: var(--green-dark);">Aperçu : {{ $exam->title }}</h5>
    </div>
    ...
    <div class="device-wrapper laptop" id="deviceWrapper">
      <div class="device-bezel">
        ...
        <div class="device-content">
          <iframe src="{{ route('cores.editor.exams.preview-iframe', $exam->id) }}" id="previewIframe" title="Exam Player Preview"></iframe>
        </div>
...
$(document).ready(function() {
  ...
  $('#btnPrintExam').click(function () {
    const printUrl = "{{ route('cores.editor.exams.print-iframe', $exam->id) }}";
