...
<body>
  ...
  <div class="no-print-toolbar no-print">
    <div>
      <h6 class="fw-bold mb-1" style="color: #1e6f5c;"><i class="bi bi-printer-fill me-1"></i> Impression de Quiz</h6>
      <p class="text-muted small mb-0">Imprimez ce document pour obtenir une version papier du quiz avec sa grille de correction à la fin.</p>
    </div>
    <button class="btn btn-primary btn-sm py-2 px-3 fw-bold" onclick="window.print()" style="background-color: #1e6f5c; border: none; border-radius: 6px; color: white; cursor: pointer;">
      <i class="bi bi-printer me-1"></i> Imprimer le Quiz
    </button>
  </div>
  ...
  <div class="print-page">
    ...
    <div>
        ...
        <div class="question-block">
          <div class="question-title">
            ...
            <div>
              {!! $q->question_text !!}
              @if(isset($q->points))
                <span class="text-muted font-normal" style="font-size: 0.85rem; font-weight: normal;">({{ $q->points }} pts)</span>
              @endif
            </div>
          </div>
      ...
      @empty
        <p class="text-muted text-center py-5">Ce quiz ne contient aucune question pour le moment.</p>
      @endforelse
    </div>
  </div>
    ...
    <div class="answer-key-list">
      @foreach($quiz->questions as $index => $q)
        ...
        <div class="answer-key-item">
          <div class="answer-key-q">Question {{ $index + 1 }} :</div>
